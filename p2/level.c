...
static void level_update_objects_bonuses() {
	for (int i = 0; i < 32; ++i) {
		...
		const int dy = obj->data.p.y_velocity >> 4;
		obj->y_pos += dy;
		int dx = obj->data.p.y_velocity + 9;
		if (dx < 256) {
			obj->data.p.y_velocity = dx;
		}
		...
		uint8_t bh = obj->y_pos >> 4;
		if (dy > 0) {
			const int tile_num = level_get_tile(bh * 256 + bl);
			uint8_t al = g_res.level.tile_attributes1[tile_num];
			if (al != 0) {
				obj->data.p.y_velocity = -obj->data.p.y_velocity;
				obj->data.p.y_velocity >>= 1;
...
static bool level_update_objects_decors_helper(struct object_t *obj) {
	...
	if (!level_objects_collide(&g_vars.objects_tbl[1], obj)) {
		return false;
	}
...
static void level_update_player_collision() {
	...
	for (int i = 0; i < 52; ++i) {
		...
		if (num == 226) {
			...
			play_sound(4);
			if (obj->data.t.unkE >= 128) {
				obj->data.t.unkE = -obj->data.t.unkE;
				int x = 32;
...
static int level_completed_bonuses_animation_helper() {
	...
	do {
		...
		for (int i = 0; i < 20; ++i) {
			...
			int y_velocity = obj->data.p.y_velocity;
			if (y_velocity < 128) {
				y_velocity += 8;
				obj->data.p.y_velocity = y_velocity;
			}
...
static void level_completed_bonuses_animation() {
	...
	while (1) {
		...
		int x_pos = 60;
		_bx = 2;
		_ax = g_vars.objects_tbl[1].x_pos - x_pos;
		if (_ax < 0) {
			_bx = -_bx;
		}
		if (_ax >= 2) {
			x_pos = g_vars.objects_tbl[1].x_pos - _bx;
			++flag;
		}
		g_vars.objects_tbl[1].x_pos = x_pos;
		...
		int y_pos = 175;
		_bx = 2;
		_ax = g_vars.objects_tbl[1].y_pos - y_pos;
		if (_ax < 0) {
			_bx = -_bx;
		}
		if (_ax >= 2) {
			y_pos = g_vars.objects_tbl[1].y_pos - _bx;
			++flag;
		}
